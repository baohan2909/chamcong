...
// [v18.132] Định kỳ đá phiên nếu mã đã chuyển (CTV→NS) — 60s/lần; hàm tự bỏ qua nếu chưa đăng nhập / không phải NV·CTV
...
function goToPage(page){
  ...
  if(page!=='tu' && page!=='tu-admin' && typeof TU!=='undefined' && TU){ try{ TU.pw=null; }catch(e){} }   // [v18.65] rời Tạm ứng → xóa mật khẩu phiên
  currentPage=page;
  ...
  // [v17.67] Khách Online: nav id 'nav-khachonline' nhưng page 'donhang-nhan'
  ...
  if(page==='chamcong' && typeof tcRefreshBanner==='function') setTimeout(tcRefreshBanner, 300); // [v17.67] Trưởng ca
  if(page==='banhang' && SESSION && SESSION.vaiTro==='CUA_HANG' && typeof tcRefreshBanner==='function') setTimeout(tcRefreshBanner, 300); // [v17.67] thẻ TC cho cửa hàng
  ...
  if(page==='tu' && typeof tuInitPage==='function')          tuInitPage();        // [v18.65] Tạm ứng nhân viên
  if(page==='tu-admin' && typeof tuAdminInitPage==='function') tuAdminInitPage(); // [v18.65] Tạm ứng quản trị
...
const HUB_GROUPS = {
  ...
  muanon: {
    ...
    items: [
      ...
      { label:'Gửi ảnh nón',        desc:'Gửi ảnh nón sưu tầm',   ic:_hubIc.img,   roles:['NV','CTV'],               quyen:'muanon.xem',       act:()=>moPageMuanon() },  // [v18.14] Aroma đổi tên; [v18.23] icon ảnh (bỏ cart)
    ]
  },
};
...
// ═════════════════════════════════════════════════════════════════════════
// [v17.67] Bộ lọc view "Sự vụ khu vực"
...
async function svcdReload(){
  ...
  if(!body) return;
  try{
    const { data, error } = await supa.rpc('fn_su_vu_co_dong_all', { p_ma_nv: SESSION.ma });
    if(error) throw error;
    _svcdData = Array.isArray(data) ? data : [];
    svcdRender();
  }catch(e){
...
function svcdFilterBar(){
  const dd = (fn,val,opts) => `<select onchange="${fn}(this.value)" style="flex:1;min-width:0;padding:8px 10px;border:1px solid #CBD5E1;border-radius:9px;font-size:13px;font-weight:600;color:#0F2E45;background:#fff">${opts.map(([v,l])=>`<option value="${v}"${val===v?' selected':''}>${l}</option>`).join('')}</select>`;
  return `<div style="display:flex;gap:8px;margin-bottom:8px">
      ${dd('svcdSetScope', _svcdScope, [['all','Tất cả sự vụ'],['mine','Của tôi'],['done','Hoàn tất']])}
      ${dd('svcdSetStatus', _svcdStatus, [['all','Mọi trạng thái'],['dang_xu_ly','Đang xử lý'],['cho_ch','Chờ CH xác nhận'],['hoan_tat','Hoàn tất']])}
    </div>
    <div style="display:flex;gap:8px;margin-bottom:12px">
      ${dd('svcdSetSort', _svcdSort, [['thoi_han','Sắp: Thời hạn'],['muc_do','Sắp: Mức độ']])}
    </div>`;
}
...
function svcdRender(){
  ...
  if(!body) return;
  const ma = SESSION.ma;
  let arr = _svcdData.slice();
  if(_svcdScope === 'mine') arr = arr.filter(s => (s.nguoi_xu_ly_ma||'') === ma);
  else if(_svcdScope === 'done') arr = arr.filter(s => s.trang_thai === 'HOAN_TAT');
  if(_svcdStatus === 'dang_xu_ly') arr = arr.filter(s => ['MOI_TAO','DA_TIEP_NHAN','DANG_XU_LY','DA_PHAN_HOI'].includes(s.trang_thai));
  else if(_svcdStatus === 'cho_ch') arr = arr.filter(s => s.trang_thai === 'DA_XU_LY_XONG');
  else if(_svcdStatus === 'hoan_tat') arr = arr.filter(s => s.trang_thai === 'HOAN_TAT');
  if(_svcdSort === 'thoi_han'){
    arr.sort((a,b)=>{
      ...
      return da !== db ? da - db : new Date(b.created_at) - new Date(a.created_at);
    });
  } else {
    const o = { KHAN_CAP:0, QUAN_TRONG:1, CAN_THIET:2 };
    arr.sort((a,b)=>{ const d = (o[a.muc_do]??9) - (o[b.muc_do]??9); return d !== 0 ? d : new Date(b.created_at) - new Date(a.created_at); });
  }
  body.innerHTML = svcdFilterBar() +
    `<div style="font-size:12px;color:#64748B;font-weight:600;margin:0 2px 10px">${arr.length} sự vụ</div>` +
    (arr.length ? arr.map(svcdCard).join('') + '<div style="height:8px"></div>'
                : '<div style="text-align:center;color:#94A3B8;padding:24px;font-size:13px;background:#fff;border-radius:10px">Không có sự vụ.</div>');
}
...
function svcdCard(s){
  ...
  if(s.trang_thai === 'HOAN_TAT'){
    ttBadge = '<span style="background:#DCFCE7;color:#15803D;font-size:11px;font-weight:700;padding:2px 8px;border-radius:20px">Hoàn tất</span>';
  } else if(s.trang_thai === 'DA_XU_LY_XONG'){
    ttBadge = '<span style="background:#FEF3C7;color:#B45309;font-size:11px;font-weight:700;padding:2px 8px;border-radius:20px">Chờ CH xác nhận</span>';
  } else {
    btn = `<button onclick="svcdXong('${s.id}')" style="width:100%;margin-top:10px;background:#0F6E56;color:#fff;border:none;padding:10px;border-radius:9px;font-weight:700;font-size:14px;cursor:pointer">Xác nhận đã xử lý xong</button>`;
  }
...
window.svcdSetSort   = function(v){ _svcdSort = v || 'thoi_han'; svcdRender(); };
...
// [v18.132] Đá phiên nếu mã đã chuyển / ngừng hoạt động — chống chấm công tiếp bằng mã cũ.
//   Boot KHÔNG tái xác thực (985) + _kiemTraDoiViTri chỉ bắt đổi role → mã đã chuyển lọt lưới.
// [v18.132] Xét theo TRẠNG THÁI (INACTIVE/DA_CHUYEN_MA), KHÔNG theo ghi_chu: mã đang dùng luôn ACTIVE,
...
// [v17.5x] Tải & hiển thị điểm phong độ của NV hiện tại lên thẻ ở trang Chấm công
async function taiDiemPhongDo(){
  const el = document.getElementById('diem-so');
  if(!el || !SESSION || !SESSION.ma) return;
  try{
    const { data, error } = await supa.rpc('fn_diem_nhan_vien', { p_ma_nv: SESSION.ma });
...
window.taiDiemPhongDo = taiDiemPhongDo;

// [v17.67] Thẻ điểm + lịch sử trừ ở tab Tài khoản (nhân viên). Cơ động → ẩn.
async function taiDiemTaiKhoan(){
  const wrap = document.getElementById('tk-diem-wrap');
  if(!wrap || !SESSION || !SESSION.ma) return;
  try{
    const { data, error } = await supa.rpc('fn_diem_chi_tiet', { p_ma_nv: SESSION.ma });
    if(error || !data || !data.success || data.co_dong){ wrap.innerHTML=''; return; }
    const diem = (data.diem != null) ? data.diem : 10;
    const sk = data.su_kien || [];
    const soTru = sk.filter(e=>!e.da_mien).length;
    const _lo = (l)=>({QUEN_RA:'Quên ra ca',QUEN_VAO:'Quên vào ca',THIEU_LICH:'Thiếu lịch',THIEU_ANH:'Thiếu ảnh',THIEU_BANGIAO:'Thiếu bàn giao',BO_SUNG:'Bổ sung ca'}[l]||l);
    const _ng = (s)=>{ if(!s)return''; const p=String(s).substring(0,10).split('-'); return p.length===3?(p[2]+'/'+p[1]):s; };
    let listHtml;
    if(!sk.length){
      listHtml = '<div style="text-align:center;color:rgba(255,255,255,.85);font-size:12.5px;padding:12px 8px 4px">Tháng này chưa bị trừ điểm nào — giữ vững nhé!</div>';
    } else {
      listHtml = sk.map(e=>{
        const off = e.da_mien;
        return '<div style="display:flex;align-items:center;gap:9px;padding:9px 2px;border-top:1px solid rgba(255,255,255,.16)">'
          + '<div style="flex:none;width:7px;height:7px;border-radius:50%;background:'+(off?'rgba(255,255,255,.4)':'#fff')+'"></div>'
          + '<div style="flex:1;min-width:0;'+(off?'opacity:.55':'')+'">'
          +   '<div style="font-size:13px;font-weight:700;color:#fff;'+(off?'text-decoration:line-through':'')+'">'+escHtml(_lo(e.loai))+'</div>'
          +   '<div style="font-size:11px;color:rgba(255,255,255,.8);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+escHtml(_ng(e.ngay))+' · '+escHtml(e.mo_ta||'')+'</div>'
          + '</div>'
          + (off ? '<div style="flex:none;font-size:10px;font-weight:700;color:rgba(255,255,255,.85);background:rgba(255,255,255,.18);padding:3px 8px;border-radius:99px">đã bỏ</div>'
                 : '<div style="flex:none;font-size:13px;font-weight:800;color:#fff">−1</div>')
          + '</div>';
      }).join('');
    }
    wrap.innerHTML = '<div class="tk-diem-card" style="margin:0 14px 14px;border-radius:20px;overflow:hidden;background:linear-gradient(135deg,#FB923C 0%,#F97316 45%,#C2410C 100%);box-shadow:0 12px 30px -12px rgba(234,88,12,.55)">'
      ...
      +     '<div style="font-size:10px;font-weight:800;letter-spacing:1px;color:rgba(255,255,255,.85)">ĐIỂM PHONG ĐỘ THÁNG NÀY</div>'
      +     '<div style="font-size:15px;font-weight:800;color:#fff;margin-top:2px">'+(soTru? ('Bị trừ '+soTru+' điểm') : 'Chưa bị trừ điểm')+'</div>'
      +   '</div>'
      + '</div>'
      + '<div style="background:rgba(0,0,0,.10);padding:6px 16px 14px">'+listHtml+'</div>'
      + '</div>';
  }catch(e){ wrap.innerHTML=''; }
}
window.taiDiemTaiKhoan = taiDiemTaiKhoan;
...
function khoiDongApp(){
  ...
  if (typeof _kiemTraHieuLucTaiKhoan === 'function') setTimeout(_kiemTraHieuLucTaiKhoan, 1800);  // [v18.132] đá phiên nếu mã đã chuyển (CTV→NS)
  ...
  if(isCH){
    ...
    // [v17.67] Tắt tính năng Lịch hoạt động → ẩn tab Lịch của cửa hàng
...
function selectCH(ma,ten){
  ...
  updateSubmitBtn();
  if(typeof tcRefreshBanner==='function') tcRefreshBanner(); // [v17.67] Trưởng ca theo cửa hàng
}
...
function doSubmit(){
  ...
  // [v17.67] Bảng hỏi Trưởng ca khi vào ca + ca chưa có TC + chưa tick nút gạt
...
// [v17.67] Ẩn dòng chấm công TỰ ĐỘNG do CHUYỂN TRƯỞNG CA khỏi HIỂN THỊ (giờ công/lịch sử NV+QL).
//          Chỉ giữ trên Google Sheet để kiểm soát. KHÔNG ẩn auto-close (quên ra).
function _ccHideAutoTc(ghiChu){
  const g = ghiChu || '';
  return /^\s*Tự động/.test(g) && (/Trưởng ca/i.test(g) || /vào ca lại/i.test(g));
}
...
function hienTomTatCa(gioRaCa){
  ...
  supa.rpc('fn_get_lich_su_hom_nay', { p_ma_nv: SESSION.ma }).then(({ data, error }) => {
    const ls = ((!error && Array.isArray(data)) ? data : []).filter(l => !_ccHideAutoTc(l.ghiChu));
...
function renderTaiKhoan(){
  ...
  // [v17.67] Thẻ điểm phong độ + lịch sử trừ
  if (typeof taiDiemTaiKhoan === 'function') taiDiemTaiKhoan();
}
...
// [v18.132] Xét THÊM trang_thai: mã ĐANG DÙNG luôn ACTIVE. Vì NV chuyển qua-lại (CTV⇄NS) nhiều lần,
...
function _loadMaChuyenSet(force){
  ...
  _maChuyenProm = supa.from('nhan_vien').select('ma_nv, ghi_chu, trang_thai')
    ...
    .then(({ data }) => {
      // [v18.132] + trang_thai: mã ACTIVE (đang dùng) KHÔNG vào Set dù ghi_chu còn dấu 'đã chuyển' cũ
