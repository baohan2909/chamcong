...
function goToPage(page){
  ...
  // [v17.60] Khách Online: nav id 'nav-khachonline' nhưng page 'donhang-nhan'
  ...
  if(page==='chamcong' && typeof tcRefreshBanner==='function') setTimeout(tcRefreshBanner, 300); // [v17.60] Trưởng ca
  if(page==='banhang' && SESSION && SESSION.vaiTro==='CUA_HANG' && typeof tcRefreshBanner==='function') setTimeout(tcRefreshBanner, 300); // [v17.60] thẻ TC cho cửa hàng
...
function _hubItemVisible(it){
  if(typeof SESSION==='undefined'||!SESSION) return false;
  if(it.setting && _getSetting(it.setting, true) === false) return false; // [v17.60] tắt theo công tắc tính năng
...
// ═════════════════════════════════════════════════════════════════════════
// [v17.60] Bộ lọc view "Sự vụ khu vực"
...
// [v17.60] Thẻ điểm + lịch sử trừ ở tab Tài khoản (nhân viên). Cơ động → ẩn.
async function taiDiemTaiKhoan(){
  ...
  try{
    ...
    const _lo = (l)=>({QUEN_RA:'Quên ra ca',QUEN_VAO:'Quên vào ca',THIEU_LICH:'Thiếu lịch',THIEU_ANH:'Thiếu ảnh',THIEU_BANGIAO:'Thiếu bàn giao'}[l]||l);
...
function khoiDongApp(){
  ...
  if(typeof tcRefreshBanner==='function') setTimeout(tcRefreshBanner, 350); // [v17.60] khôi phục thẻ Trưởng ca + hero khi tải lại
  ...
  if(isCH){
    ...
    // [v17.60] Tắt tính năng Lịch hoạt động → ẩn tab Lịch của cửa hàng
...
function selectCH(ma,ten){
  ...
  updateSubmitBtn();
  if(typeof tcRefreshBanner==='function') tcRefreshBanner(); // [v17.60] Trưởng ca theo cửa hàng
}
...
function doSubmit(){
  ...
  // [v17.60] Bảng hỏi Trưởng ca khi vào ca + ca chưa có TC + chưa tick nút gạt
...
function renderTaiKhoan(){
  ...
  // [v17.60] Thẻ điểm phong độ + lịch sử trừ
