...
function goToPage(page){
  ...
  if(page==='chamcong' && typeof tcRefreshBanner==='function') setTimeout(tcRefreshBanner, 300); // [v17.32] Trưởng ca
  if(page==='banhang' && SESSION && SESSION.vaiTro==='CUA_HANG' && typeof tcRefreshBanner==='function') setTimeout(tcRefreshBanner, 300); // [v17.32] thẻ TC cho cửa hàng
...
function _hubItemVisible(it){
  if(typeof SESSION==='undefined'||!SESSION) return false;
  if(it.setting && _getSetting(it.setting, true) === false) return false; // [v17.32] tắt theo công tắc tính năng
...
function khoiDongApp(){
  ...
  if(isCH){
    ...
    // [v17.32] Tắt tính năng Lịch hoạt động → ẩn tab Lịch của cửa hàng
...
function selectCH(ma,ten){
  ...
  updateSubmitBtn();
  if(typeof tcRefreshBanner==='function') tcRefreshBanner(); // [v17.32] Trưởng ca theo cửa hàng
}
...
function doSubmit(){
  ...
  // [v17.32] Bảng hỏi Trưởng ca khi vào ca + ca chưa có TC + chưa tick nút gạt
